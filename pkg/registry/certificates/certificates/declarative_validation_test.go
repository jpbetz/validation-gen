...
func testDeclarativeValidation(t *testing.T, apiVersion string) {
	...
	for k, tc := range testCases {
		t.Run(k, func(t *testing.T) {
			apitesting.TestDeclarativeValidation(t, ctx, &tc.input, Strategy.Validate, tc.expectedErrs)
		})
	}
}
	...
	for k, tc := range testCases {
		for _, subresource := range tc.subresources {
			t.Run(k+" subresource="+subresource, func(t *testing.T) {
				...
				apitesting.TestDeclarativeUpdateValidation(t, ctx, &tc.update, &tc.old, strategy.ValidateUpdate, tc.expectedErrs)
			})
		}
	}
}
