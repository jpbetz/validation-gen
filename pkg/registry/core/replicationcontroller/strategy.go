...
func (rcStrategy) Validate(ctx context.Context, obj runtime.Object) field.ErrorList {
	...
	if utilfeature.DefaultFeatureGate.Enabled(features.DeclarativeValidation) {
		...
		validation_identifier := "replication_controller_create"

		// Run declarative validation with panic recovery
		declarativeErrs := rest.ValidateDeclaratively(ctx, legacyscheme.Scheme, controller, rest.WithTakeover(takeover), rest.WithValidationIdentifier(validation_identifier))
...
func (rcStrategy) ValidateUpdate(ctx context.Context, obj, old runtime.Object) field.ErrorList {
	...
	if utilfeature.DefaultFeatureGate.Enabled(features.DeclarativeValidation) {
		// Determine if takeover is enabled
		takeover := utilfeature.DefaultFeatureGate.Enabled(features.DeclarativeValidationTakeover)
		validation_identifier := "replication_controller_update"

		// Run declarative update validation with panic recovery
		declarativeErrs := rest.ValidateUpdateDeclaratively(ctx, legacyscheme.Scheme, newRc, oldRc, rest.WithTakeover(takeover), rest.WithValidationIdentifier(validation_identifier))
