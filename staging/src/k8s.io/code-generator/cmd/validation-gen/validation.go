...
var (
	...
	validatePkgSymbols  = mkPkgNames(validatePkg, "EachMapValNilable", "EachSliceVal")
)
	...
	case types.Struct:
		for _, fld := range thisNode.fields {
			...
			if fld.node != nil {
				...
				case types.Map:
					// Map of slices where the slice element has validations.
					if sliceElemNode := getSliceElementNode(fld); sliceElemNode != nil && g.hasValidations(sliceElemNode) {
						emitComments(fld.fieldValidations.Comments, bufsw)
						if !fldRatchetingChecked {
							emitRatchetingCheck(c, fld.childType, bufsw)
							fldRatchetingChecked = true
						}
						g.emitMapOfSliceValidation(c, sliceElemNode, bufsw)
					}
...
// emitMapOfSliceValidation generates validation code for map[K][]T where T has validations
func (g *genValidations) emitMapOfSliceValidation(c *generator.Context, sliceElemNode *typeNode, sw *generator.SnippetWriter) {
	targs := generator.Args{
		"sliceElemType": sliceElemNode.valueType,
		"validateFunc":  c.Universe.Type(sliceElemNode.funcName),
	...
	sw.Do("errs = append(errs, $.validate.EachMapValNilable|raw$(\n", targs)
	sw.Do("    ctx,op,fldPath,obj,oldObj,\n", targs)
	sw.Do("    func(ctx $.context.Context|raw$, op $.operation.Operation|raw$, ", targs)
	sw.Do("         fldPath *$.field.Path|raw$, obj, oldObj []$.sliceElemType|raw$) $.field.ErrorList|raw$ {\n", targs)
	sw.Do("        return $.validate.EachSliceVal|raw$(ctx, op, fldPath, obj, oldObj, nil, $.validateFunc|raw$)\n", targs)
	sw.Do("    })...)\n", targs)
}
...
// isMapOfSlices checks if the given childNode represents a map whose values are slices
func isMapOfSlices(child *childNode) bool {
	return child.node != nil &&
		child.node.elem != nil &&
		child.node.elem.node != nil &&
		child.node.elem.node.valueType.Kind == types.Slice
}

// getSliceElementNode returns the element node of a slice type within a map value,
// or nil if the structure doesn't match map[K][]T
func getSliceElementNode(mapChild *childNode) *typeNode {
	if !isMapOfSlices(mapChild) {
		return nil
	}
	elemNode := mapChild.node.elem.node
	if elemNode.elem != nil && elemNode.elem.node != nil {
		return elemNode.elem.node
	}
	return nil
}
