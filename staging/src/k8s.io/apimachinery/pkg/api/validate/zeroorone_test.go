...
func TestZeroOrOneOfUnion(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			expected:    field.ErrorList{field.Invalid(nil, "{b, d}", "must specify at most one of: `a`, `b`, `c`, `d`").WithOrigin("zeroOrOneOfMember")},
		},
		{
			name:        "all members set",
			fields:      [][2]string{{"a", "A"}, {"b", "B"}, {"c", "C"}, {"d", "D"}},
			fieldValues: []bool{true, true, true, true},
			expected:    field.ErrorList{field.Invalid(nil, "{a, b, c, d}", "must specify at most one of: `a`, `b`, `c`, `d`").WithOrigin("zeroOrOneOfMember")},
		},
		{
...
func TestZeroOrOneOfUnionRatcheting(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			expected: field.ErrorList{
				field.Invalid(nil, "{m1, m2}", "must specify at most one of: `m1`, `m2`").WithOrigin("zeroOrOneOfMember"),
			},
		},
		{
